...
/*
 * [UPGRADE 1] Cluster secret verified by the leader on every MSG_AUTH
 * connection. The sender must embed this in the MSG_AUTH payload.
 * Change this to any shared passphrase before deployment.
 */
#define CLUSTER_SECRET "osgroup4"
...
typedef enum {
  ...
  /*
   * [UPGRADE 2] Force-local execution request.
   * Sender uses "local <path>" command. Leader skips load balancer and
   * runs the job on itself regardless of available remote workers.
   */
  MSG_LOCAL_EXEC_REQ = 24,  /* dispatcher → leader: run locally        */
} MsgType;
...
/*
 * [UPGRADE 1] Auth payload sent by sender inside MSG_AUTH.
 * Contains both the legacy "HELLO" greeting and the cluster secret so
 * the leader can validate in one read without a second round-trip.
 */
typedef struct {
  char greeting[8];       /* "HELLO\0\0\0"                            */
  char secret[64];        /* CLUSTER_SECRET null-terminated           */
} AuthPayload;
